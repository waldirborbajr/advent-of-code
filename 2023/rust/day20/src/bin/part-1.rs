...
fn main() {
    let input = Input::from_file(format!("{}/input.txt", env!("CARGO_MANIFEST_DIR")))
...
fn part2(input: &Input) -> usize {
    ...
    for name in &input.modules[BROADCASTER_MODULE].targets {
        ...
        while let Some(next) = input.modules[cur]
            .targets
            .iter()
            .find(|&m| input.modules[m].is_flipflop())
        {
            chain.push(next);
            cur = next;
        }
    ...
    let periods: Vec<_> = flipflop_chains
        .into_iter()
        .map(|chain| {
            chain.into_iter().rev().fold(0, |acc, k| {
                (acc << 1)
                    + if input.module_targets_conjunction(&k) {
                        1
                    } else {
                        0
                    }
...
impl ModuleNetwork {
    fn new(modules: HashMap<String, Module>) -> Self {
        ...
        for (name, module) in &modules {
            for target in module.targets.iter().filter(|t| {
                modules
                    .get(t.as_str())
                    .map(|m| m.is_conjunction())
                    .unwrap_or_default()
            }) {
    ...
    fn press_button(&mut self) {
        ...
        *self
            .low_pulses
            .entry(BROADCASTER_MODULE.to_owned())
            .or_default() += 1;

        let mut queue: VecDeque<_> = [(BUTTON_MODULE, BROADCASTER_MODULE, Pulse::Low)]
            .into_iter()
            .collect();
        while let Some((from, to, pulse_in)) = queue.pop_front() {
            ...
            let pulse_out = match module.module_type {
                ...
                ModuleType::Conjunction => {
                    ...
                    Some(if memory.values().all(|&p| matches!(p, Pulse::High)) {
                        Pulse::Low
                    } else {
                        Pulse::High
                    })
...
impl Input {
    ...
    fn module_targets_conjunction(&self, name: &str) -> bool {
        self.modules[name]
            .targets
            .iter()
            .any(|n| matches!(self.modules[n].module_type, ModuleType::Conjunction))
    }
